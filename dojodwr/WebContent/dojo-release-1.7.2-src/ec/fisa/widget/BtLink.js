define([
		...
		function(declare,Link, dojo, template, domGeometry, has) {
			...
			return declare("ec.fisa.widget.BtLink", [Link ], {
				btId:'',
				fieldId:'',
				tabId:'',
				pageScopeId:'',
				...
				onClick:function(){
					...
					var notificationVal = controller.addNotificationValDataForLinkAction();
					messagesPanel.clearAllMessages();
					BtControllerDWR.execLink(this.btId,this.btPos,this.actionMode,this.fieldId,this.tabId,this.pageScopeId, controller.model.toPlainObject(),notificationVal,{callbackScope:this,callback:this.handleButtonResponse});
				},
...
//						var newSize = domGeometry.getMarginBox(borderContainerTwo.domNode);
						...
						if(format=="pdf"){
						  ...
						  if(has("webkit") && format == "pdf"){
							  ...
							  var dialog = ec.fisa.navigation.utils.openHrefFisaDlgPopupUnique("popUpBtLinkUnique","",href,{content:args});
							  if(format=="pdf"){
						...
						var aditionalParams = {content:{linkParentFisaTabId:this.tabId, linkParentFisaPageScopeId:this.pageScopeId, linkParentBtId:this.btId}};
