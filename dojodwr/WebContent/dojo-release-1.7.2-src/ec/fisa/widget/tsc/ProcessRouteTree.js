define([ "dojo/_base/declare", 
	...
	return declare("ec.fisa.widget.tsc.ProcessRouteTree", [ Tree ], {
		tabId : '',
		pageScopeId : '',
		actionId:'',
		copyOnly : true,
		...
		checkItemAcceptance : function(node, source, position) {
			...
			if ((elmParent == "rule" || elmParent == "root") && (position == "before" || position == "after")) {
				return false;
			}
		...
		checkItemTreeAceptance:function(){
			ProcessRouteControllerDWR.getProcessRouteElements({
				callbackScope : this,
		...
		getElementAttributeData:function(){
			ProcessRouteControllerDWR.getAttributesGroupByElement({
				callbackScope : this,
				callback : function(data) {
		...
		createInitialAttributesTable:function(){
			...
			if (this.actionId == "IN" || this.actionId == "UP") {
				this.gridLayout = [
				{ name: 'Propiedad', field: 'attr', editable: false,width: "150px" /* Can't edit ID's of dojo/data items */ },
				{ name: 'Valor',field: 'widget',width : "250px",'editable':false,cellType:dojox.grid.cells._Widget,widgetClass: dijit.form.TextBox,
